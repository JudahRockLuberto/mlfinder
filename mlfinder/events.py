...
class FindEvents():
    def __init__(self, bd, fields, precision):
        ...
        self.stars.ra = list(self.stars.ra) - self.bd.ra
        self.stars.dec = list(self.stars.dec) - self.bd.dec
    ...
    def close_star_find(self, a_ends, d_ends, radius):
        ...
        columns = list(self.stars.columns)
    ...
    #
    #
    def plot_event_path(self, zoom=0.2, years=1, figsize=(10,10), gaia_check=False):
         # basic setup
        ...
        if gaia_check == True:
            ...
            gaia_handle = mpatches.Patch(color='red', label=r'\textit{Gaia} point source')
            not_gaia_handle = mpatches.Patch(color='grey', label=r'Not a \textit{Gaia} point source')
        ...
        bs_plot = ax1.scatter(self.stars.ra, self.stars.dec, s = 10, c = gaia_c)
        
        # plot the brown dwarf path
        ax1.scatter(self.coord_df.ra, self.coord_df.dec, s=3, c='orange')
    ...
    def centroid_shift(self, which, masses=None):
        ...
        for index, row in self.bd.coord_df.iterrows():
            ...
            for j in range(len(mjups)):
                theta = pyasl.getAngDist(row.ra, row.dec, self.event_table.ra[which], self.event_table.dec[which])
