...
class Fields():
    def __init__(self, file=None, stars_df=None, ra = None, dec = None, bd = None, n_arcmin=5):
        ...
        if isinstance(stars_df, (Table, pd.DataFrame)):
            self.stars = pd.DataFrame(stars_df)
        
        if file == None and stars_df == None:
            q = """SELECT
